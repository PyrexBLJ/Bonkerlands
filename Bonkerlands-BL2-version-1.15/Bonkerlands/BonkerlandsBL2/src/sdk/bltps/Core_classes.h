...
//char waujduaw[offsetof(UObject, UObject::Name)];
// Class Core.Object
// 0x003C
class UObject
{
...
public:
	static TArray< UObject* >* UObject::GObjObjects()
	{
		TArray< UObject* >* ObjectArray = (TArray< UObject* >*) GObjects;
		return ObjectArray;
	}

	template<class T> static std::vector<T*> FindAll(char* InStr)
	{
		std::vector<T*> ret;
		for (size_t i = 0; i < GObjObjects()->Count; ++i)
		{
			UObject* object = GObjObjects()->Data[i];

			if (object && object->Class)
			{
				if (!strcmp(object->Class->GetName(), InStr))
	...
	template<class T> static T* FindObject(int index)
	{
		...
		T* t = (T*)UObject::GObjObjects()->Data[index];
		if (t)
			return t;
	...
	template< class T > static T* FindObject(char* ObjectFullName)
	{
		...
		for (int i = 0; i < UObject::GObjObjects()->Count; ++i)
		{
			...
			// check 
			if (!strcmp(Object->GetFullName(), ObjectFullName))
					return (T*)Object;
		}

		return NULL;
	}
	static UClass* FindSubClass(char* ClassFullName);
	...
	static UObject* GetInstanceOf(UClass* pClass)
	{
		UObject* pObjectInstance = 0;
		if (UObject::GObjObjects())
		{
			for (int i = 0; i < UObject::GObjObjects()->Count; ++i)
			{
				...
				{
					UObject* pCheckObject = UObject::GObjObjects()->Data[i];
					if (pCheckObject)
					{
