...
char* UObject::GetName()
{
	...
	_sys_sprintf(cOutBuffer, "%s", this->Name.GetName());
	return cOutBuffer;
}
...
char* UObject::GetNameCPP()
{
	...
	if (this->IsA(UClass::StaticClass()))
	{
		...
		while (pClass)
		{
			...
			{
				std::strcpy(cOutBuffer, "A");
				break;
			}
			else if (!strcmp(pClass->GetName(), "Object"))
			{
				std::strcpy(cOutBuffer, "U");
				break;
			}
	...
	{
		std::strcpy(cOutBuffer, "F");
	}

	std::strcat(cOutBuffer, this->GetName());
...
{ 
	...
	{ 
		...
		if ( this->Outer->Outer ) 
		{ 
			std::strcpy ( cOutBuffer, this->Class->GetName());
			std::strcat ( cOutBuffer, " " ); 
			std::strcat ( cOutBuffer, this->Outer->Outer->GetName() ); 
			std::strcat ( cOutBuffer, "." ); 
			std::strcat ( cOutBuffer, this->Outer->GetName() ); 
			std::strcat ( cOutBuffer, "." ); 
			std::strcat ( cOutBuffer, this->GetName() ); 
		} 
		else 
...
UClass* UObject::FindSubClass(char* ClassFullName)
{
	...
	for (int i = 0; i < UObject::GObjObjects()->Count; ++i)
	{
		...
		if (!strcmp(Object->GetFullName(), ClassFullName))
...
class UObject* UObject::FindObject ( struct FString ObjectName, class UClass* ObjectClass )
{
	...
	if ( ! pFnFindObject )
		pFnFindObject = (UFunction*) UObject::GObjObjects()->Data[2759];
...
bool UObject::IsA ( struct FName ClassName )
{
	...
	if ( ! pFnIsA )
		pFnIsA = (UFunction*) UObject::GObjObjects()->Data[3274];
...
void UObject::GetAxes ( struct FRotator A, FVector* X, FVector* Y, FVector* Z )
{
	
	UFunction* pFnGetAxes = NULL;

	if ( ! pFnGetAxes )
		pFnGetAxes = (UFunction*) UObject::GObjObjects()->Data[3502];
...
float UObject::Dot_VectorVector ( struct FVector A, struct FVector B )
{
	...
	if ( ! pFnDot_VectorVector )
		pFnDot_VectorVector = (UFunction*) UObject::GObjObjects()->Data[3645];
...
struct FVector UObject::Subtract_VectorVector ( struct FVector A, struct FVector B )
{
	...
	if ( ! pFnSubtract_VectorVector )
		pFnSubtract_VectorVector = (UFunction*) UObject::GObjObjects()->Data[3665];
...
float UObject::Tan ( float A )
{
	...
	if ( ! pFnTan )
		pFnTan = (UFunction*) UObject::GObjObjects()->Data[3792];
