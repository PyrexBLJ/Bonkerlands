...
int __cdecl scheduler::UObject_ProcessEvent(UObject* obj, UFunction* f, void* params, void* a)
{
	if (f)
	{
		...
		/*if (strstr(str, "Function WillowGame.WillowPlayerController.PlayGfxMovieDefinition")) {
			if (obj) {
				auto* moviedef = (AWillowPlayerController_execPlayGfxMovieDefinition_Parms*)obj;
				printf("Process Event PlayGfxMovieDefinition: %s\n", moviedef->MovieDefinitionName.c_str());
			}
		}*/
...
int  __cdecl UObject_CallFunction(UObject* obj, void* frame, void* a3, UFunction* func)
{
	...
	/*if (strstr(functionName, "Function WillowGame.WillowPlayerController.PlayGfxMovieDefinition")) {
		if (obj) {
			auto* moviedef = (AWillowPlayerController_execPlayGfxMovieDefinition_Parms*)obj;
			printf("Call Function PlayGfxMovieDefinition: %s\n", moviedef->MovieDefinitionName.c_str());
		}
	}*/

	/*if (strstr(functionName, "Function Engine.WorldInfo.GetStreamingPersistentWorldInfo")) {
		if (obj) {
			auto* world2 = (AWorldInfo_execGetStreamingPersistentWorldInfo_Parms*)obj;
			bltps::render_t::worldinfo = (AWorldInfo*)world2->ReturnValue;
			printf("got worldinfo from stream\n");
		}
	}*/

	if (strstr(functionName, "Function Engine.Engine.GetEngine")) {
		if (obj) {
			auto* engine = (UEngine_execGetEngine_Parms*)obj;
			bltps::render_t::UEngine = (UEngine*)engine->ReturnValue;
			//printf("got UEngine\n");
		}
	}
