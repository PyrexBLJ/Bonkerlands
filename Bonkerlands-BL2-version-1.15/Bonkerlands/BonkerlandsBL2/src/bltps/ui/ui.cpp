...
namespace bltps {
	...
	void runPatch() {
		...
		file_name = buf;
		appLoadFileFromString_d = new detour(appLoadFileFromString_a, appLoadFileFromString);//0x0061360
	...
	void PlaySound(char *name) {
		...
		/*UAkEvent* ev = UObject::Find*/
		UAkEvent* event = UObject::FindObject<UAkEvent>(name);
		printf("Found Object, Trying to Play\n"); //got here
		render_t::UEngine->GamePlayers.Data[0]->Actor->PlayAkEvent(event, 0,0,1,0);

		//render_t::controller->PlayAkEvent(event, 1, 1, 1, 1);
		printf("Sound Done\n");
	}

	
	/*static char* s_buffer = nullptr;
	void call_patch(const std::string& name)
	{
		if (bltps::render_t::engine == nullptr)
			return;

		FString CString;

		auto file_size = libpsutil::filesystem::get_file_size(BONKERLANDS_DIR + name);

		if (file_size == 0 || file_size == -1)
			return;

		if (sys_memory_allocate(20 * 1024 * 1024, SYS_MEMORY_PAGE_SIZE_1M, (sys_addr_t*)&s_buffer) > 0)
			return;

		libpsutil::filesystem::read_file(BONKERLANDS_DIR + name, s_buffer, file_size);
		if (s_buffer[0] == '\0')
			return;

		appBufferToString(&CString, s_buffer, file_size);

		static symbol<bool(wchar_t** a1, FString* a2, unsigned int a3)> ParseLine{ ParseLine_a };
		static symbol<void(ULocalPlayer* local, wchar_t*, void* fcc)> ExecuteCommand{ ExecuteCommand_a };

		auto* ulocal_player = render_t::engine->GetPlayerOwner(0);
		FString parsed_string;
		while (ParseLine(&CString.Data, &parsed_string, 0))
		{
			auto* line = (wchar_t*)parsed_string.Data;
			ExecuteCommand(ulocal_player, line, reinterpret_cast<void*>(*reinterpret_cast<uintptr_t*>(FOutput_Logf_Ptr)));
		}
	...
	void ui::on_build() {
		...
		vars->menu.clr_accent = color(192, 92, 92, 255);
		...
		save_menu->add_option(new menu_item(L"Player Level", &vars->playerlevel, 1, 1, 72, []() {
			if (InGame()) {
				//level
				render_t::hud->WPRI->ExpPointsNextLevelAt = vars->playerlevel;
				//max level xp
				render_t::hud->WPRI->ExpPointsNextLevelAtBaseValue = render_t::controller->GetExpPointsRequiredForLevel(vars->playerlevel);
				//rank up
		...
		save_menu->add_option(new menu_item(L"Skill Points", &vars->skillpoints, 1, 0, 72, []() {
			if (InGame()) {
				render_t::hud->WPRI->SpecialistSkillPoints = vars->skillpoints;
				toast::show(L"Bonkerlands", L"Skill Points Set", 3500, nullptr);
			}
			else {
				toast::show(L"Bonkerlands", L"Must Be In-Game", 3500, nullptr);
			}
			}, type_integer_none, L"Should Have 68 at lvl 72"));
		save_menu->add_option(new menu_item(L"Set Money", &vars->money, 1000, 0, 99999999, []() {
			if (InGame()) {
				render_t::hud->WPRI->Currency[0].LastKnownAmount = vars->money;
		...
		save_menu->add_option(new menu_item(L"Set Eridium", &vars->eridium, 1, 0, 500, []() {
			if (InGame()) {
				render_t::hud->WPRI->Currency[1].LastKnownAmount = vars->eridium;
				toast::show(L"Bonkerlands", L"Eridium Set", 3500, nullptr);
			}
			else {
				toast::show(L"Bonkerlands", L"Must Be In-Game", 3500, nullptr);
			}
			}, type_integer_none));
		save_menu->add_option(new menu_item(L"Set Seraph Crystals", &vars->seraph, 1, 0, 999, []() {
			if (InGame()) {
				render_t::hud->WPRI->Currency[2].LastKnownAmount = vars->seraph;
				toast::show(L"Bonkerlands", L"Seraph Crystals Set", 3500, nullptr);
			}
			else {
				toast::show(L"Bonkerlands", L"Must Be In-Game", 3500, nullptr);
			}
			}, type_integer_none));
		save_menu->add_option(new menu_item(L"Set Torgue Tokens", &vars->Torgue, 1, 0, 999, []() {
			if (InGame()) {
				render_t::hud->WPRI->Currency[4].LastKnownAmount = vars->Torgue;
				toast::show(L"Bonkerlands", L"Torgue Tokens Set", 3500, nullptr);
			}
			else {
				toast::show(L"Bonkerlands", L"Must Be In-Game", 3500, nullptr);
			}
		...
		//		/*render_t::controller->SetBadassPoints(1);
		//		render_t::controller->SetBadassTokens;*/
	...
	void ui::on_run() {
		if (this->get_open()) {
			...
			else if (is_down(btn_square, CELL_PAD_BTN_OFFSET_DIGITAL2))
			{
				...
				libpsutil::sleep(150);
				//PlaySound("AkEvent Ake_UI.UI_Generic.Ak_Play_UI_Generic_Confirm");
			}
			...
			libpsutil::sleep(150);
				...
				libpsutil::sleep(150);
				//PlaySound("AkEvent Ake_UI.UI_Generic.Ak_Play_UI_Generic_Back");
			}
				...
				if (menu)
				{
					...
					if (!wcscmp(menu->get_name(), L"Save")) {
						if (InGame()) {
							...
							vars->skillpoints = render_t::hud->WPRI->SpecialistSkillPoints;
							vars->money = render_t::hud->WPRI->Currency[0].LastKnownAmount;
							vars->eridium = render_t::hud->WPRI->Currency[1].LastKnownAmount;
							vars->seraph = render_t::hud->WPRI->Currency[2].LastKnownAmount;
							vars->Torgue = render_t::hud->WPRI->Currency[4].LastKnownAmount;
						}
					}
